<?php
    ...
    foreach ($galleryImages as $imgPath) {
        $processedGalleryImages[] = file_exists(public_path($imgPath)) ? asset($imgPath) : ($mediaMap[$imgPath] ?? 'https://picsum.photos/seed/' . md5($imgPath) . '/800/600');
    }
?>
<!--
...
>
    ...
    >
        ...
        >
            ...
            <button @click="closeModal()" class="absolute -top-4 -right-4 h-10 w-10 bg-secondary rounded-full flex items-center justify-center text-primary hover:text-accent transition-colors duration-300">
